...
fn test_socket_base_dir_uses_tmp() {
    ...
    assert_eq!(
        result,
        std::path::PathBuf::from(format!("/tmp/agent-procs-{}", uid))
    );
}
...
fn test_socket_path() {
    ...
    assert_eq!(
        result,
        std::path::PathBuf::from(format!("/tmp/agent-procs-{}/mysession.sock", uid))
    );
}
...
fn test_pid_path() {
    ...
    assert_eq!(
        result,
        std::path::PathBuf::from(format!("/tmp/agent-procs-{}/mysession.pid", uid))
    );
}
...
fn test_state_dir_defaults_to_home() {
    ...
    let expected = std::path::PathBuf::from(format!(
        "{}/.local/state/agent-procs/sessions/test-session",
        home
    ));
...
fn test_socket_path_is_short() {
    ...
    let path =
        agent_procs::paths::socket_path("my-long-session-name-that-would-have-failed-before");
    assert!(
